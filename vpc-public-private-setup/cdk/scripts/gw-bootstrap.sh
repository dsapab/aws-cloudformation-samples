...
#
# Exactly two forward paths ever match. One is out tun0, the private subnets into
# the tunnel. The other is out eth0 back into 10.0.0.0/16, the established replies
# returning to the VPC. eth0 has to stay in the forward path, since it is the
# ingress for client traffic and the egress for return traffic, so it can't be
# turned off. Forwarding out eth0 to anything outside the VPC matches no rule and
# hits DROP. There is deliberately no eth0 SNAT either, so an unmatched packet
# leaves un-NATed and the internet gateway discards it.
...
# outbound, private subnets may only egress into the tunnel
iptables -A FORWARD -s 10.0.0.0/16 -o tun0 -j ACCEPT
# return, established replies may only go back into the VPC, never to the internet
iptables -A FORWARD -o eth0 -d 10.0.0.0/16 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
# NAT only through the tunnel (no eth0 SNAT exists, so nothing can leak out eth0)
iptables -t nat -A POSTROUTING -s 10.0.0.0/16 -o tun0 -j MASQUERADE
